...
|------|------|----------|-------|
| **Covenant** | The constitution. Dignity, Sovereignty, Consent. | Prose | `Covenant/` |
| **Omninet** | The protocol. 29 Rust crates, 6,700+ tests. Complete. | Rust | `Omninet/` |
| **Omny** | The browser. Throne (Tauri 2.x shell) + Castle (contract engine) + Chancellor (state authority). Programs in `Apps/`. | Rust + TypeScript | `Omny/` |
| **Library** | Shared libraries. SDK + UI + Crystal + Editor + FX. | TypeScript + WGSL | `Library/` |

**Data flows:** Omninet → (FFI) → Chancellor → (IPC) → Throne → (bridge) → Library SDK → Apps programs
...
The C header (`divinity_ffi.h`) auto-generates via cbindgen. The SDK codegen script (`Library/sdk/scripts/generate.py`) parses this header to produce TypeScript types and operations automatically.

---

## Layer 3: Daemon Module (Omny → Chancellor)

### Where daemon modules live

`Omny/chancellor/chancellor/src/modules/`
...
1. Create `Omny/chancellor/chancellor/src/modules/your_mod.rs`
...
4. Add to module list in `Omny/chancellor/chancellor/src/modules/mod.rs`
...
## Layer 4: API Contract (Omny → chancellor/api_json.rs)
...
```json
{"auth": "<hex-token>", "client_type": "throne", "program_id": null}
→ {"auth": "ok", "session_id": "abc123", "client_type": "throne"}
```
...
## Layer 6: JS Bridge (Omny → Throne/bridge → window.omninet)
...
1. **Tauri invoke()** — native IPC in Throne (primary, no HTTP roundtrip)
...
**But the SDK disagrees.** `Library/sdk/src/bridge.ts` declares `run(pipeline: PipelineRequest)` taking an **object**, and `ops.ts` passes objects directly (not JSON strings). The Cornerstone bridge correctly uses JSON strings.
...
Both editor and programs capture `window.omninet` method references at module load time via `.bind()` and freeze them. Malicious code replacing `window.omninet` later can't affect existing programs.

---

## Layer 7: SDK (Library → @omnidea/net)

### Auto-generated from Rust

`Library/sdk/scripts/generate.py` parses:
...
3. Run `cd Library/sdk && npm run generate`
4. The new operation appears in TypeScript automatically

---

## Layer 8: UI Components (Library → @omnidea/ui)

### 54 Solid.js components
...
## Layer 9: Programs (Apps/)

### What programs are

Manifest-driven Solid.js TypeScript applications. They are "lenses" — views into Omnidea's data, not siloed apps. Each program has a `program.json` manifest declaring its Court permissions, Library packages, and intents.
...
Apps/
  hearth/
    program.json   ← Manifest (court, packages, intents)
    Hearth.tsx     ← Entry component
    store.ts       ← Reactive state (Solid.js signals)
...
1. Create `Apps/your-program/program.json` (manifest)
2. Create `Apps/your-program/YourProgram.tsx` (entry component)
3. Declare Court permissions in the manifest
4. Use `@castle/cornerstone` for the `useCastle()` hook
5. Use `@omnidea/ui` components for UI
6. Run `npx crownsguard check ../../Apps/your-program/` to validate
...
### 3. Library — SDK regeneration
```bash
cd Library/sdk && npm run generate
# bookmark.create now appears in @omnidea/net automatically
```

### 4. Omny — Daemon module handler
**File:** `Omny/chancellor/chancellor/src/modules/yoke_mod.rs` (add handler)
```rust
state.phone.register_raw("bookmark.create", move |data| {
...
### 5. Omny — API contract
**File:** `Omny/chancellor/chancellor/src/api_json.rs` (add converter)
```rust
...
| Library (SDK codegen) | Python | `cd sdk && npm run generate` |
| Library (SDK compile) | TypeScript | `cd sdk && tsc` |
| Library (Crystal) | Vite | `cd crystal && vite build` |
| Library (UI CSS) | tsx | `cd ui && tsx scripts/build-css.ts` |
| Library (all) | npm workspaces | `npm run build` (sdk → crystal → ui) |
| Omny (Chancellor) | Cargo | `cd chancellor && cargo build` |
| Omny (Throne) | Tauri | `cd throne && cargo tauri dev` |
| Tests (Rust) | Cargo | `cargo test` (from Omninet root) |
| Tests (TypeScript) | Vitest | `cd Library && npx vitest run` |

**Build order:** Omninet → Library SDK → Library UI → Chancellor → Throne
...
| Wiring to daemon | `Omny/chancellor/chancellor/src/modules/{domain}_mod.rs` |
| Controlling JSON shape | `Omny/chancellor/chancellor/src/api_json.rs` |
| Updating SDK types | `cd Library/sdk && npm run generate` (automatic) |
| Building UI | `Apps/{program}/*.tsx` + `Library/ui/src/**/*.tsx` |
| Adding a component | `Library/ui/src/lib/{atoms\|components\|layout}/*.tsx` |
| Editor features | `Library/editor/src/*.ts` + `Omny/chancellor/chancellor/src/modules/editor_mod.rs` |
| Glass/Crystal effects | `Library/crystal/src/*.ts` |
| Theme/tokens | `Library/ui/src/lib/theme/` + Regalia crate |
