...
```bash
# Omninet (protocol) -- 6,700+ tests
cd Omninet && cargo test --workspace

# Library (shared libraries) -- TypeScript tests via vitest
cd Library && npm test

# Omny (browser) -- Chancellor Rust tests
cd Omny/chancellor && cargo test
```
...
2. **Name which repo(s) are affected.** A change in Omny might need awareness of Omninet, Library, or the Covenant.
